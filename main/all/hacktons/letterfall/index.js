...
class TextTerrain extends THREE.Object3D {
  constructor(anisotropy) {
    ...
    g.setAttribute("letterIdx", new THREE.InstancedBufferAttribute(new Float32Array(Array.from({ length: tileDim * tileDim }, () => {
    ...
    this.actions = Array.from({ length: 5000 }, () => {
      let action = (delay) => {
        ...
        let tween = new TWEEN.Tween({ val: 0 }).to({ val: 1 }, 10000)
          .delay(delay)
...
scene.background = new THREE.Color("#000"); // Black background
